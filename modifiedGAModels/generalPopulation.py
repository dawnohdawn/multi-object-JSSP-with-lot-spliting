...
class generalPopulation:
    ...
    def iterate_MBO(self, iterNum, K, S, M, needcalAllMakespan=1, muteEveryIter=0, muteResult=0, **kw):
        """
        功能：              简单GA迭代，可对同一个population对象连续使用
                            首次使用应把needcalAllMakespan设为1，后面应设为0以减少重复计算
    ...
    def iterate_NIMGA(self, popIndex, iterNum, p1, p2, p3, needcalAllMakespan=1, muteEveryIter=0,
                 muteResult=0, **kw):
        """
        功能：              简单GA迭代，可对同一个population对象连续使用
                            首次使用应把needcalAllMakespan设为1，后面应设为0以减少重复计算
        ...
        输出：
        changeFlag          经过这么多代进化之后，最优目标值如果有变好，则返回1，否则返回0
        ...
        for iterInd in range(iterNum):
            ...
            for i in range(int(self.popSize / 2)):
                ...
                if (random.random() < p1):
                    ...
                    child1.crossoverBetweenSegment2s(child2, 0.5)
                # # segment1变异
                for item in [child1, child2]:
                    if random.random() < p2:
                        if (random.random() < 0.5):
                            # item.mutateSegment1WithTwoSublots(ps3)
                            item.neighbourResizeTwoSublot()
                        else:
                            # item.mutateSegment1WithNewVec(ps4)
                            item.nerghbourIncreaseOrDecreaseASublotNum()
                # segment2变异
                for item in [child1, child2]:
                    if random.random() < p3:
                        if popIndex == 0:
                            # item.mutateSegment2WithinVecWithSwap(ps5)
                            item.neighbourInverseLotsOfAMachine()
                        elif popIndex == 1:
                            # item.mutateSgment2BetweenTwoVecs()
                            item.neighbourRepositionALot()
                        elif popIndex == 2:
                ...
                # 不择优
                # makespanList = [item.makespan for item in self.pop]
                # minMakespan = self.getBestMakespan()
                # if parentsIndexs[i * 2] == makespanList.index(minMakespan):
                #     newPop.extend([parent1, child2])
                # elif parentsIndexs[i * 2 + 1] == makespanList.index(minMakespan):
                #     newPop.extend([parent2, child1])
                # else:
            ...
            # # 更新changeFlag
            # if min([item.makespan for item in newPop]) < self.getBestMakespan():
            #     changeFlag = 1

            # 强行把最好的一个保留下来，随即替换一个
            makespanList = [item.makespan for item in self.pop]
            newPop[random.randint(0, self.popSize - 1)] = copy.deepcopy(self.pop[makespanList.index(self.getBestMakespan())])
...
#
#
# test = generalPopulation(51, lotNum, lotSizes, machineNum, generalIndividual, generalSolution)
